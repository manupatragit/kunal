//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace DataAccess.Modals
{
    using System;
    
    public partial class usp_GetNoticeByID_Result
    {
        public string NoticeID { get; set; }
        public string CaseStatus { get; set; }
        public string NoticeType { get; set; }
        public Nullable<System.DateTime> NoticeCreatedOn { get; set; }
        public Nullable<System.DateTime> DateofNotice { get; set; }
        public string FirmId { get; set; }
        public string UserId { get; set; }
        public string ModeofServiceorDelivery { get; set; }
        public string AddressedTo { get; set; }
        public string AddresseeAddress { get; set; }
        public string OtherDetailsofAddressee { get; set; }
        public string SendersName { get; set; }
        public string SendersAddress { get; set; }
        public string OtherDetailsofSender { get; set; }
        public string NoticeSubject { get; set; }
        public string NoticeTitle { get; set; }
        public string CreateNotice { get; set; }
        public string NoticeThrough { get; set; }
        public System.DateTime DateofDelivery { get; set; }
        public System.DateTime DateofReceipt { get; set; }
        public string RespondentsName { get; set; }
        public string RespondentsAddress { get; set; }
        public string NoticeReference { get; set; }
        public string ReplySubject { get; set; }
        public string CreateReply { get; set; }
        public string OtherDetailsofRespondent { get; set; }
        public string NoticeReceivedbyClient { get; set; }
        public string NoticeBroughtbyClientforReply { get; set; }
        public System.DateTime ReplytoNoticeCreatedOn { get; set; }
        public System.DateTime DateofReply { get; set; }
        public string ReplyAddressedto { get; set; }
        public System.DateTime DateofReplytoNotice { get; set; }
        public System.DateTime DateofReceivingReply { get; set; }
        public string ModeofReceipt { get; set; }
        public string ModeofDeliveryofRejoinder { get; set; }
        public System.DateTime DateofCreatingRejoinder { get; set; }
        public string ReplyThrough { get; set; }
        public System.DateTime DateofServiceofNotice { get; set; }
        public System.DateTime DateofDispatchofNotice { get; set; }
        public System.DateTime DateofRejoinder { get; set; }
        public string RejoinderAddressedto { get; set; }
        public string NoticeandReplyReference { get; set; }
        public string RejoinderSubject { get; set; }
        public string CreateRejoinder { get; set; }
        public string RejoinderThrough { get; set; }
        public string TypeofNotice { get; set; }
        public string ApprovedForDispatch { get; set; }
        public bool ApproveByManager { get; set; }
        public bool ApproveByClient { get; set; }
        public string AssignmentofTask { get; set; }
        public System.DateTime GenerationofAlerts { get; set; }
        public string CurrentStatus { get; set; }
        public System.DateTime RecordCreated_Date { get; set; }
        public string AmountClaimed { get; set; }
        public System.DateTime ActualDateOfClosure { get; set; }
        public string Criticality { get; set; }
        public string Tag { get; set; }
        public string SonOf { get; set; }
        public string RoOf { get; set; }
        public string Rejoinder { get; set; }
        public string Resonforhighpriority { get; set; }
        public string MainNoticeId { get; set; }
        public int IsFileAvailable { get; set; }
        public string ParentNoticeId { get; set; }
        public string NoticeThroughId { get; set; }
        public string SubectId { get; set; }
        public string RootNoticeId { get; set; }
        public string CreatedByName { get; set; }
        public string col1 { get; set; }
        public string col2 { get; set; }
        public string col3 { get; set; }
        public string col4 { get; set; }
        public string col5 { get; set; }
        public string col6 { get; set; }
        public string col7 { get; set; }
        public string col8 { get; set; }
        public string col9 { get; set; }
        public string col10 { get; set; }
        public string col11 { get; set; }
        public string col12 { get; set; }
        public string col13 { get; set; }
        public string col14 { get; set; }
        public System.DateTime DueDateOfNotice { get; set; }
        public string Modeofservicedeliverylist { get; set; }
        public string NoticeThroughIdlist { get; set; }
        public string Senderothertxt { get; set; }
        public string SenderNameId { get; set; }
        public string SentThrough { get; set; }
        public string MailSubject { get; set; }
        public string NoticeSharedSubject { get; set; }
        public string SentToManagerId { get; set; }
        public string FeedbackMailSubject { get; set; }
        public string SharedSendersName { get; set; }
        public string SharedReceiverName { get; set; }
        public string SentToClientId { get; set; }
    }
}
