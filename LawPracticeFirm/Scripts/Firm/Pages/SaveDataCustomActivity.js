$(document).ready(function () {
    ...
    $(document).on("click", "#gocreateact", function () {
        var token = $(this).attr("data-val");
        localStorage.setItem("catoken", id);
        window.location = encodeURI(token);
    });
    //load Assign User
    $("#contact").change(function () {
        ...
        if (clientds != "") {
            ...
            $('#case').empty().append('<option value="">Select Case</option>').find('option:first').attr("selected", "selected");
        }
    });
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.Data == "[]") {
                    ...
                    if (String(roleid) == "1") {
                        $("#contenterrormsg").html("<a id='gocreateact' style='color:black' class='btn btn-default' title='click here to add custom field' href='javascript:void()' data-val='/" + fcode + "/Firm/CreateCustomActivity'>Please add custom fields.</a>").css("display", "block").css("cusror", "pointer").css("color", "red !important");
                    }
                    else if (String(roleid) == "2") {
                        $("#contenterrormsg").html("Please contact admin to add custom field.").css("display", "block").addClass("btn btn-default").css("color", "black !important");
                    }
                }
                else {
                    $("#submitform").css("display", "block");
                    $("#contenterrormsg").text("").css("display", "none");
                }
                $.each(obj, function (i, field) {
                    ...
                    if (field["FieldType"] == " 11") {
                        ...
                        $.each(selectvalues1.split(','), function (key, value1) {
                            if (key == "0") {
                                ...
                                html += '<label class="radio-inline"><input type = "radio" data-taskid="' + field["Id"] + '"  class="gender  dymenu"   placeholder="' + field["FieldName"] + '" value="' + value1 + '" name = "gender"  ' + requireds + ' id="demotext' + sum + '"  > ' + value1 + '</label>';
                            }
                        });
                    ...
                    if (field["FieldType"] == " 1") {
                        ...
                        $.each(selectvalues1.split(','), function (key, value) {
                            html += '<p class="checkbox-inline zyx"><span>' + value + '</span> <input class="demotext' + sum + ' clist  dymenu"  data-taskid="' + field["Id"] + '" myclass="clist" type = "checkbox" placeholder="' + field["FieldName"] + '" id="demotext' + sum + '" name="checkbox' + field['Id'] + '" value="' + value + '" ' + requireds + '  ></p >';
                        });
                        $("#content").append(html + '<span id="idss" value="' + field['Id'] + '" style="float:left;padding:8px 0 0 2px;cursor:context-menu" class="teempclass"></span>');
                    }
                    //client
    ...
    $('form[id="savecustomform"]').validate({
        submitHandler: function (form) {
            ...
            formData.append("caid", id);
            openloader();
            $.ajax(
                {
                    type: "POST",
                    url: "/api/Callapi/SaveCustomActivitydata", // Controller/View
                    dataType: 'json',
                    headers: {
                        "ctxt1": ctx1,
                        "ctxt2": ctx2,
