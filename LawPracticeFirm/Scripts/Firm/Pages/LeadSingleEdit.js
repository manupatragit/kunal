            ...
            success: function (response) {
                if (response.Status == true) {
                    ...
                    $.each(obj1, function (i, b) {
                        ...
                        try {
                            if (b.ldctype != null) {
                                var option = '<option value="' + b.ldctype + '" selected > ' + b.ldctype + '</option>';
                                $("#lbtyte").append(option);
                            }
                            if (b.ldsource != null) {
                                if ($('#lsource option[value="' + b.ldsource + '"]').length > 0) {
                                    $('#lsource option[value="' + b.ldsource + '"]').attr("selected", true);
                                }
                                else {
                                    $("#lsource option[value='Others']").attr("selected", true);
                                    $("#olsource").val(b.ldsource);
                                    $("#olsource").css("display", "block");
                                }
                            }
                            if (b.ldcategory != null) {
                                if ($('#lcategory option[value="' + b.ldcategory + '"]').length > 0) {
                                    $('#lcategory option[value="' + b.ldcategory + '"]').attr("selected", true);
                                }
                                else {
                                    $("#lcategory option[value='Others']").attr("selected", true);
                                    $("#olcategory").val(b.ldcategory);
                                    $("#olcategory").css("display", "block");
                                }
                            }
                            if (b.ldltype != null) {
                                if ($('#ltype option[value="' + b.ldltype + '"]').length > 0) {
                                    $('#ltype option[value="' + b.ldltype + '"]').attr("selected", true);
                                }
                                else {
                                    $("#ltype option[value='Others']").attr("selected", true);
                                    $("#oltype").val(b.ldltype);
                                    $("#oltype").css("display", "block");
                                }
                            }
                            var option4 = '<option value="' + b.ldcountry + '" selected > ' + b.ldcountry + ' </option>';
                            $("#lcountry").append(option4);
                            if (b.ldcountry == "India") {
                                $("#ostate").css("display", "none");
                                $("#ocity").css("display", "none");
                                $("#lstate").css("display", "unset");
                                $("#lcity").css("display", "unset");
                                var option = '<option value="' + b.ldstate + '" selected > ' + b.ldstate + ' </option>';
                                $("#lstate").append(option);
                                var option1 = '<option value="' + b.ldcity + '" selected > ' + b.ldcity + ' </option>';
                                $("#lcity").append(option1);
                            }
                            else {
                                ...
                                $("#ostate").val(b.ldstate);
                                $("#ocity").val(b.ldcity);
                            }
                            $("#lname").val(b.ldname);
                            $("#cperson").val(b.ldcperson);
                            $("#org").val(b.ldorg);
                            $("#lpost").val(b.lddesign);
                            $("#laddress").val(b.ldadd);
                            $("#lpin").val(b.ldpin);
                            $("#lemail").val(b.ldemail);
                            $("#lphn").val(b.ldphn);
                            $("#lmob").val(b.ldmob);
                            $("#lplan").val(b.ldplan);
                            $("#lec").val(b.ldec);
        ...
        submitHandler: function (form) {
            ...
            if (lname == "") {
                ...
                $.ajax(
                    {
                        type: "POST",
                        url: "/api/LeadApi/UpdateLead", // Controller/View
                        dataType: 'json',
                        headers: {
                            "ctxt1": ctx1,
                            "ctxt2": ctx2,
                        ...
                        success: function (data) {
                            if (data.Data == "Already Exists Mobile Please Try Another Mobile!") {
                                ...
                                window.location = encodeURI("/" + fcode + "/Firm/ViewLead");
                            }
                        }, //End of AJAX Success function
                        failure: function (data) {
                            alert(data.responseText);
