$(document).ready(function () {
    ...
    /*Export fav contact in excel*/
    $("#excel").click(function () {
        var searchdata = $('#searchdata').val();
        window.location = encodeURI("/firm/ExportoExcelFavContact?status=true&searchdata=" + searchdata + "&exportfilter=" + exportfilter);
    })
    /*Export fav contact in pdf*/
    $("#pdf").click(function () {
        var searchdata = $('#searchdata').val();
        window.location = encodeURI("/firm/ExportoPdfFavContact?status=true&searchdata=" + searchdata + "&exportfilter=" + exportfilter);
    })
    /*Single Contact View*/
    ...
    $(document).on("click", "#syncrqst", function () {
        ...
        if (result) {
            ...
            if (JSON.stringify(selectedIDSync) != "[]") {
                ...
                formdata.append("tablekey", "fcontact");
                openload();
                $.ajax({
                    async: true,
    ...
    $(document).on("click", "#linkFav", function () {
        ...
        function Favcontact() {
            ...
            if (JSON.stringify(fselectedID) == "[]") {
                new PNotify({
                    title: 'Warning',
                    text: 'You do not have selected any row to un-favourite.',
                    type: 'error',
                    delay: 3000
                });
                return false;
            }
            var result = confirm("Are you sure to remove contact from favourite.?");
            if (result) {
                $('input:checkbox.checkbox').each(function () {
                    if ($(this).prop('checked')) {
                        fselectedID.push($(this).val());
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            if (response.Status == true) {
                                ...
                                new PNotify({
                                    title: 'Success!',
                                    text: ' Contacts removed from favourite successfully.',
                                    type: 'success',
                                    delay: 3000
                                });
                                $('#select_all').prop('checked', false);
                                ...
                                new PNotify({
                                    title: 'Warning!',
                                    text: ' You are not Authotized, remove to favourite this Contact!',
                                    type: 'error',
                                    delay: 2000
                                });
                                closeload();
    ...
    /*Get contact details on key press*/
    ...
    function loadcontactlist(pageindex) {
        ...
        var ld12 = $.ajax({
            ...
            success: function (response) {
                ...
                console.log("favcontact:" + totalTime);
                $("#tfooter").html("");
                $("#tokens").html("");
                if (response.Status == true) {
                    var datas = JSON.stringify(response);
