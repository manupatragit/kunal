...
$(document).ready(function () {
    ...
    function GeneratedItemsTable4() {
        if (payorderItems.length > 0) {
            ...
            $table4.append('<thead><tr><th><div class="thbg">Date</div></th><th><div class="thbg">Payment Mode</div></th><th><div class="thbg">Pay Received</div></th><th><div class="thbg">Bank Name</div></th><th><div class="thbg">DD No</div></th><th><div class="thbg">DD Date</div></th><th><div class="thbg">Cheque No</div></th><th><div class="thbg">Cheque Date</div></th><th><div class="thbg">Ref No</div></th><th><div class="thbg">Other Details</div></th><th><div class="thbg">Action</div></th></tr></thead>');
            var $tbody4 = $('<tbody/>');
            $.each(payorderItems, function (i, val) {
                ...
                if (val.Ddidate != "") {
                    $row4.append($('<td/>').html(formatDatetoIST(val.Ddidate)));
                }
                else {
                    $row4.append($('<td/>').html(val.Ddidate));
                }
                $row4.append($('<td/>').html(val.ChequeNo));
                if (val.Chqidate != "") {
                    $row4.append($('<td/>').html(formatDatetoIST(val.Chqidate)));
                }
                else {
                    $row4.append($('<td/>').html(val.Chqidate));
                }
                    $row4.append($('<td/>').html(val.RefNo));
                    $row4.append($('<td/>').html(val.OtherDetails));
                var $remove4 = $('<a href="#" ><img src="/newassets/img/deletecasesingle-icon.png" /></a>');
                $remove4.click(function (e) {
                    var result = confirm("Are you sure to delete invoice ?");
                    if (result) {
                        e.preventDefault();
            ...
            $("#showvirtualTable").show();
        }
        else {
            $('#orderItems4').html('');
            $("#showvirtualTable").hide();
        }
    }
    $("#createpay").click(function () {
        ...
        $.ajax({
            url: '/api/OcrInvoiceApi/PostSavePayment',
            data: formData,
            contentType: false,
            processData: false,
            type: 'POST',
            success: function (response) {
                if (response.Status == true) {
                    ...
                    if (response.Data == "can not blank") {
                        ...
                        alert("Paid amount can not be more than due amount")
                    }
                    else {
                        new PNotify({
                            title: 'Success!',
