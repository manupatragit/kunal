$(document).ready(function () {
        ...
        if (pageindex != "undefined") {
            if (Math.sign(pageindex) == 1) {
                ...
                if (pageindex <= parseInt(pageindesx)) {
                    openload();
                    $("[data-type='accordion-ordering']").siblings('i').find(".fa-sort").remove();
    ...
    function loadtabledata(pageindex) {
        ...
        formdata.append("search", $('#searchtext').val().replace(/"/g, "").replace(/'/g, ""));
        $.ajax({
            async: true,
                ...
                try {
                    $.each(obj, function (i, val) {
                        ...
                        var $row1 = '<div data-type="accordion-section" id="maindiv' + val.mid + '" class="search " style="cursor: pointer;border-left: 1px solid gray;border-right: 1px solid gray;border-bottom: 1px solid lightgray; overflow:hidden;margin:0 0 5px 0;" >' +
                            ...
                            ' <button type="button" id="replyAll" idval="' + val.mid + '" class="sbtbtn"><i class="fa fa-reply"></i> Reply All</button>' +
                            ' <button type="button" id="reply" idval="' + val.mid + '" class="sbtbtn"><i class="fa fa-reply"></i> Reply</button>' +
                            ' <button type="button" id="forword" idval="' + val.mid + '" class="sbtbtn"><i class="glyphicon glyphicon-share-alt"></i> Forward</button>' +
                            ' <button type="button" class="sbtbtn" id="print1" data-toggle="tooltip" title="Print" idval="' + val.mid + '"><i class="glyphicon glyphicon-print"></i>&nbsp;</button>' +
                            '' + deletebtn + ' <button type="button" id="archivemsg" idval="' + val.mid + '" class="sbtbtn"><i class="fa fa-archive"></i> Archive</button>' +
                            ...
                            '</div>' +
                            '<div style="widht:100%">' +
                            '<button type="button" id="savereplyAll" idval="' + val.mid + '" userval="' + val.userid + '" class="sbtbtn save ash' + val.mid + '" style="padding:5px; !Important" >Send All</button>' +
                            '<button type="button" id="savereply" idval="' + val.mid + '" userval="' + val.userid + '" class="sbtbtn save sh' + val.mid + '" style="padding:5px; !Important">Send</button>' +
                            '&nbsp;&nbsp;&nbsp;<button type="button" id="closereply" class="sbtbtn"  idval="' + val.mid + '" style="padding:5px; !Important">Close</button>' +
                            '</div>' +
                            ' </div>' +
                            ' </div>' +
                            ...
                            '<p id="case' + val.mid + '"> <b>Matter : </b> <span> ' + val.mattername + '</span></p>' +
    ...
    $(document).on("click", "#forword", function () {
        ...
        //CKEDITOR.instances['details'].setData(df);
                            ...
                            try {
                                if (String(mfile) != "") {
                                    ...
                                    for (var i = 0; i < str_array2.length; i++) {
                                        ...
                                        //alert($row);
                                        //  if (str_array1.length<=)
                                        $("#viewattachfilereplyAll" + val.mid + val.id).append($row2);
                                        //$(".shfile2" + val.mid + val.id + q2).not(':first').remove();
                                    }
                                }
                            }
                            catch (err) {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.Status == true) {
                    ...
                    $("#touserreply" + desireds).text(hbind.replace(/,\s*$/, ""));
