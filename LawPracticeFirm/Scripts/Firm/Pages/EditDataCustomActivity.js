            ...
            success: function (response) {
                if (response.Status == true) {
                    ...
                    $.each(obj1, function (i, b) {
                        try {
                            $("#subject").val(b.subject);
                            if (b.caseid != null) {
                                var option12 = '<option value="' + b.caseid + '" selected > ' + b.mattername + ' </option>';
                                $("#case").append(option12);
                            }
                        } catch (err) {
                            // alert(err.message);
                        }
            ...
            success: function (data) {
                ...
                $.each(obj, function (i, field) {
                    ...
                    if (field["FieldType"] == "19") {
                        sum = sum + 1;
                        var textBox = '<div class="form-group"><label class="control-label col-md-4" >' + field['FieldName'] + ' <span class="required  rqd' + field['Id'] + '"   aria-required="true">*</span> </label ><div class="col-md-8"><input class="form-control dymenu" data-taskid="' + field["Id"] + '" placeholder="' + field["FieldName"] + '" type="file" multiple="multiple"  value=""  id="demotext' + sum + '"  name="demoname"><span id="idss" value="' + field['Id'] + '" style="float:left;padding:8px 0 0 2px;cursor:context-menu" class="teempclass"></span></div></div>';
                        $("#content").append(textBox);
                    }
                    // textbox
    ...
    $('form[id="savecustomform"]').validate({
        submitHandler: function (form) {
            ...
            $.ajax(
                {
                    type: "POST",
                    url: "/api/Callapi/EditCustomActivitydata", // Controller/View
                    dataType: 'json',
                    headers: {
                        "ctxt1": ctx1,
                        "ctxt2": ctx2,
                    ...
                    success: function (data) {
                        ...
                        closeload();
                        if (String(roleid) == "1") {
                            window.location = encodeURI("/" + fcode + "/Firm/Activities/");
                        }
                        else if (String(roleid) == "2") {
                            window.location = encodeURI("/" + fcode + "/Employee/Activities/");
                        }
                    },
                    failure: function (data) {
                        alert(data.responseText);
