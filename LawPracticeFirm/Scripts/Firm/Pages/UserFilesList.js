$(document).ready(function () {
    ...
    $(document).on("click", "#bruidcmb", function () {
        ...
        var xu = $(this).attr("data-target");
        var xt = "collapseExample" + xu;
    ...
    var j6 = "";
    function loadcontactpermission() {
        var cvalue = 66;
        if (cvalue != "") {
                        ...
                        for (var i = 0; i < strarray.length; i++) {
                            if ($("#target" + tfid).attr('perm' + tfid) == tfid) {
                                $("#checkboxes" + tfid + " input[name='options[]']").each(function () {
                                    var chkbx = $(this).val();
                                    if (strarray[i] == chkbx) {
                                        $("#checkboxes" + tfid + " input[name='options[]']" + "#chkd" + chkbx).attr('checkeddata', true);
                                        ...
                                        // alert(chkbx);
                                        //alert("no");
                                        //$("#checkboxes" + tfid + " input[name='options[]']" + "#chkd" + chkbx).prop('checked', false);
                                        // $("#checkboxes" + tfid + " input[name='options[]']" + "#chkd" + chkbx).attr('checkeddata', false);
                                    }
                                });
                            }
                        }
    ...
    function permission() {
        $.ajax({
            ...
            success: function (response) {
                ...
                $.each(response.Data, function (i, a) {
                    var option = '&nbsp;&nbsp;<input type="checkbox" name="options[]" id="chkd' + a.Id + '" value="' + a.Id + '"  disabled  />  <span style="visibility:hidden">' + a.Pname + '</span>';
    ...
    function LoadDirectory() {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $.each(response.Data, function (i, a) {
                    var tmpdir = a.fname;
                    var dirname = tmpdir.replace("WorkSpace/1/", "")
                    var option = '<option value="' + a.Id + '" > ' + dirname + '</option>';
                    $("#directory").append(option);
                    var option = '<option value="' + dirname + '" > ' + dirname + '</option>';
    ...
    function LoadDirectoryFiles() {
        try {
            ...
            var j1 = $.ajax({
                async: true,
                type: "POST",
                url: "/api/callApi/UserDirectoryList",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (response1) {
                    ...
                    $.each(obj, function (i, a) {
                        ...
                        html += '<div class="col-md-5 "><span style="cursor:pointer"  data-target="' + a.id + '" aria-expanded="false" aria-controls="collapseExample" id="bruidcmb" value="' + a.fname + '"><span class="glyphicon glyphicon-folder-open" aria-hidden="true" style="color:orange"></span>&nbsp;&nbsp; ' + dirname + '</span></div>';
                        html += '<div class="col-md-2">' + dates1 + '</div>';
                        html += ' <div class="col-md-1"> <span class="glyphicon glyphicon-trash cls' + a.id + '" style="color:red; cursor:pointer" id="removedir" values="' + a.id + '" dirname="' + a.fname + '"> </span> </div>';
                        html += '<div class="col-md-4"><span class="linkdata1" id="checkboxes' + a.id + '"></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div>';
                        html += ' </div > <br>';
                        html += ' <div class="col-md-12 collapse"  dtr="collapseExample' + a.id + '"  id="collapseExample' + a.id + '" style="display:none !important"   tot="' + a.id + '" dirname="' + a.fname + '"> </div>';
                    });
            ...
            $.when(j1).then(function (a1) {
                ajax1();
                loadcontactpermission();
                $.when(j6).then(function (j6) {
                    $.ajax({
                        async: true,
                        type: "POST",
                        url: "/api/callApi/DirectoryList",
                        ...
                        success: function (response1) {
                            ...
                            var cntn = 0;
                            $.each(response1.Data, function (i, av) {
                                cntn = cntn + 1;
                                var rvalue = $(".cls" + av.Id).attr("values");
                                var ds = $("#checkboxes" + rvalue + " " + "input[name = 'options[]']" + "#chkd3").attr("checkeddata");
                                if (ds == "true") {
                                    $(".cls" + rvalue).css("display", "block");
                                    $(".cls" + rvalue).show();
                                }
                                else {
                                    $(".cls" + rvalue).css("display", "none");
                                    $(".cls" + rvalue).hide();
                                }
                                setTimeout(function () {
                                    var rs = $("#checkboxes" + rvalue + " " + "input[name = 'options[]']" + "#chkd1").attr("checkeddata");
                                    if (rs == "true") {
                                        $(".clsdown" + rvalue).css("display", "block");
                                        $(".clsdown" + rvalue).show();
                                    }
                                    else {
                                        $(".clsdown" + rvalue).css("display", "none");
                                        $(".clsdown" + rvalue).hide();
                                    }
                                }, 2000);
                                setTimeout(function () {
                                    var rs3 = $("#checkboxes" + rvalue + " " + "input[name = 'options[]']" + "#chkd2").attr("checkeddata");
                                    if (rs3 == "true") {
                                        $(".clsrmv" + rvalue).css("display", "block");
                                        $(".clsrmv" + rvalue).show();
                                    }
                                    else {
                                        $(".clsrmv" + rvalue).css("display", "none");
                                        $(".clsrmv" + rvalue).hide();
                                    }
                                }, 2000);
                            });
        ...
        var j2 = $.ajax({
            ...
            success: function (response) {
                ...
                $.each(response.Data, function (i, a) {
                    ...
                    if (align == a.pfile) {
                        $('#collapseExample' + a.pfile).append('<div style="padding:0 0 3px 0; overflow:hidden;font-size:14px;"><div class="col-md-6" style="padding: 0 0 0 35px;"><i class="fa fa-mail-forward" style="color:grey"></i> &nbsp;<i class="' + ficon + '" style="font-size:14px;color:' + icolor + '"></i><span style="cursor:pointer" id="idss"  value="' + last + '"  href="' + bsurlfile + '/Docs/ViewSample?fileName=' + a.fname + '&dname=' + dirname + '"> ' + a.fname + '</span></div><div class="col-md-2">' + last + '</div><div class="col-md-2">' + dates2 + '</div><div class="col-md-1"> <span class="glyphicon glyphicon-trash clsrmv' + a.pfile + '" style="color:maroon; cursor:pointer" id="removedirfile" directnamefull="' + fullfilepath + '" values="' + a.Id + '"> </span>&nbsp;&nbsp;&nbsp;<div class="col-md-1"> <a class="glyphicon glyphicon-download-alt clsdown' + a.pfile + '" style="color:green; cursor:pointer" download="' + a.fname + '" id="downloadfile" href="' + downloadpath + '" values="' + a.Id + '"> </a> </button></div></div></div> ');
                    }
                    else {
                        alert("Try Again !");
                    }
