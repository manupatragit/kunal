function CustomField() {
    ...
    var delegateMethods = {
        ...
        hideModal: function () {
            if (document.activeElement.id !== "btnClose") {
                ...
                if (privateVariables.configuredcustomfields.length > 0) {
                    tempId = Enumerable.From(privateVariables.configuredcustomfields).Min(function (a) {
                        return tempId === -1 ? -1 : a.Id - 1;
                    });
                    seq = Enumerable.From(privateVariables.configuredcustomfields).Max(function (a) {
                        return a.Sequence + 1;
                    });
                }
                var field = {
        ...
        saveConfiguration: function () {
            ...
            if (privateMethods.validateConfiguration((isPublished && parseInt(Id, 10) > 0))) {
                var status = true;
                if (configurationtype === 6) {
                    if (!$("#txtformname")[0].validity.valid) {
                        alert("Form Name can not be blank!!");
                        $("#txtformname").focus();
                        status = false;
                    }
                }
                if (status) {
                    $.each($(">div", $("#divCFields")), function (i, a) {
                        ...
                        if (obj.length > 0) {
                            obj[0]["Sequence"] = i + 1;
                            obj[0]["FieldValues"] = $.isArray(obj[0]["FieldValues"]) ? obj[0]["FieldValues"].join(',') : "";
                        }
                    });
                    privateVariables.configuredcustomfields = Enumerable.From(privateVariables.configuredcustomfields)
                        .OrderBy(function (a) {
                    ...
                    $.each(privateVariables.configuredcustomfields, function (i, a) {
                        a["FieldValues"] = $.isArray(a["FieldValues"]) ? a["FieldValues"].join(',') : "";
                    });
                    Utility.postDataToServer("api/FirmApi/SaveFirmCustomFields", { "FormId": parseInt(Id, 10), "IsPublished": isPublished, "FormName": $.trim($("#txtformname").val()), "CustomFieldList": privateVariables.configuredcustomfields }, { "configurationtype": configurationtype }, "json", privateMethods.saveCustomField, null, null, true);
                }
            } else {
                alert("No New Field Added!!");
            }
        }
    };
    var privateMethods = {
        ...
        getConfiguredCustomField: function () {
            // alert(ConfigurationType);
            Utility.postDataToServer("api/FirmApi/ConfiguredCustomFields", {}, { "configurationtype": configurationtype, "id": Id }, "json", privateMethods.loadConfiguredCustomField, null, null, false);
        },
        loadConfiguredCustomField: function (response) {
            alert(JSON.stringify(response));
            if (response.Status) {
                privateVariables.configuredcustomfields = response.Data.CustomFieldList;
                alert(configurationtype);
                if (configurationtype === 6) {
                    $("#txtformname").val(response.Data.FormName);
                    if (response.Data.IsPublished) {
                        $("#btnSave,#btnPublish").remove();
                    }
                }
                $.each(privateVariables.configuredcustomfields, function (i, a) {
                    alert(privateVariables.configuredcustomfields);
                    var obj = Enumerable.From(privateVariables.customfields)
                        .Where(function (n) {
                            return n["Id"] === a["FieldType"];
                        }).ToArray();
                    if (obj.length > 0) {
                        CustomFields.addCustomField(a, "divCField" + a["Id"], !a["IsDefault"], a["IsSortable"]);
                    }
                });
            }
        },
        validateConfiguration: function (skipnewfield) {
            var result = true;
            var newFields = $.grep(privateVariables.configuredcustomfields, function (n, i) {
                return n["Id"] < 0;
            });
            if (newFields.length === 0 && !skipnewfield) {
                result = false;
            }
            return result;
        },
        addCustomField: function (field, id) {
            ...
            if (field["FieldType"] === 12 || field["FieldType"] === 13 || field["FieldType"] === 14 || field["FieldType"] === 19) {
        ...
        initiatePlugins: function () {
            ...
            $("#divCFields").sortable({ placeholder: "ui-state-highlight", cancel: ".ui-state-disabled" });
        },
        attachevents: function () {
            ...
            $("#btnSave,#btnPublish").unbind().bind("click", delegateMethods.saveConfiguration);
            $("#btnCancel").unbind().bind("click", delegateMethods.cancel);
            $("#txtformname").unbind().bind("dblclick", function () {
                $(this).removeClass("borderless");
            })
                .bind("focus", function () {
                    $(this).removeClass("borderless");
                    if ($.trim($("#txtformname").val().toUpperCase()) === 'Form Name') {
                        $("#txtformname").val('');
                    }
                });
        },
