$(document).ready(function () {
    var cacpttoken = localStorage.getItem("catoken");
    loadform();
    var fcode = localStorage.getItem("FirmCode");
    setInterval(function () {
        var temp23 = localStorage.getItem("Customactivity1");
        if (temp23 != "") {
            loadform();
            localStorage.setItem("Customactivity1", "");
        }
    }, 2000);
    ...
    $("#addcontacttype").click(function () {
        openload();
        var url = "/" + fcode + "/firm/CreateActivity/";
    ...
    $('#formtype').on('change', function () {
        ...
        if (selectid == "") {
            new PNotify({
                title: 'Warning!',
                text: ' Select Activity Name',
                type: 'error',
                delay: 3000
            });
            closeload();
            $("#staticdata1").css("display", "none");
            $("#staticdata2").css("display", "none");
            $("#staticdata3").css("display", "none");
            $("#staticdata4").css("display", "none");
            $("#content").css("display", "none");
            $("#addcustomfiled").css("display", "none");
        }
        else {
            $("#staticdata1").css("display", "block");
            $("#staticdata2").css("display", "block");
            $("#staticdata3").css("display", "block");
            $("#staticdata4").css("display", "block");
            $("#content").css("display", "block");
            $("#addcustomfiled").css("display", "block");
            loadfield();
        }
    });
    ...
    function CheckPublishForm() {
        var sstype = localStorage.getItem("type");
        var j8 = $.ajax({
            async: true,
            type: "POST",
            url: "/api/CallApi/CheckCustomActivityPublish",
            headers: {
                'formid': sstype
            },
            contentType: "application/json; charset=utf-8",
            ...
            success: function (response) {
                ...
                $.each(response.Data, function (i, a) {
                    if (a.savecount != "0") {
                        check = true;
                        $(".glyphicon-trash").css("display", "none");
                        $('.dymenu').removeClass('dymenu').addClass('dymenu1');
                        $("#content").css("cursor", "unset");
                        $("#addcustomfiled").css("display", "none");
                    }
                    else {
                        check = false;
                        $(".glyphicon-trash").css("display", "block");
                        $('.dymenu1').removeClass('dymenu1').addClass('dymenu');
    ...
    function loadform() {
        openload();
        $("#formtype").html("");
        var option1 = '<option value="" selected >Select Activity</option>';
        ...
        var f1 = $.ajax({
            ...
            success: function (response) {
                ...
                console.log("loadtime:" + totalTime)
                if (response.Status == true) {
                    var datas = JSON.stringify(response);
                ...
                $.each(obj, function (i, a) {
                    var option = '<option value="' + a["Id"] + '" > ' + a["ActivityName"] + '</option>';
        ...
        $.when(f1).then(function (data, textStatus, jqXHR) {
            if (cacpttoken != "" || cacpttoken != null || cacpttoken != "null" || cacpttoken != "NULL" || cacpttoken != "Null") {
                $('#formtype option[value="' + cacpttoken + '"]').attr("selected", true);
                var selectid = $('#formtype').val();
                stype = selectid;
                localStorage.setItem("type", stype);
                localStorage.setItem("catoken", "");
                if (selectid == "") {
                    closeload();
                    $("#staticdata1").css("display", "none");
                    $("#staticdata2").css("display", "none");
    ...
    function loadmenu() {
        $.ajax({
            ...
            success: function (response) {
                ...
                $.each(response.Data, function (i, a) {
                    var t = a.Id;
                    if (String(a.Text) != "Accept / Reject") {
    ...
    $("#addfield").click(function () {
        ...
        if (formtype == "") {
            $(".validpanel").css("display", "block").html("  Select Activity Name to Add Custom Field !");
            new PNotify({
                title: 'Success!',
                text: ' Select Activity Name to Add Custom Field !',
                type: 'error',
                delay: 3000
            });
        }
