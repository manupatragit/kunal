            ...
            success: function (response) {
                if (response.Status == true) {
                    ...
                    $.each(obj, function (i, val) {
                        stritem += '<tr id="opendata" dataval="' + enctype(val.taskid) + '" style="cursor:pointer"><td class="taskid" id=' + val.id + ' >' + val.WorkFlowName + '</td><td>' + val.WFDescription + '</td><td>' + formatDatetoIST(val.CreatedOn) + '</td><td ' + q1 + '">' + val.Task + '</td><td ' + q1 + '">' + val.TaskDescription + '</td><td ' + q1 + '">' + formatDatetoIST(val.DueDate) + '</td></td></tr>';
                        stritem += '<tr><td colspan=6><div id=div' + enctype(val.taskid) + '></td></tr>';
                    });
                    stritem += "</tbody>";
...
function GetStageDetailBytaskid(strtaskid) {
    ...
    var stritem = '';
    var q1 = 0;
    strtable = $('<table id=divstage' + strtaskid + ' style="overflow-x:auto;width:95%; border:2px solid #204D74" align="center" /><tr><th>').addClass('dataTable table-responsive table table-bordered table-striped');
    $.ajax(
        {
            type: "POST",
            ...
            dataType: 'json',
            data: formData,
            headers: { 'uid': '1', 'taskid':dtoe(strtaskid) },
            contentType: 'application/json;charset=utf-8',
            processData: false,
            success: function (response) {
                if (response.Status == true) {
                    var datas = JSON.stringify(response);
                    var obj = JSON.parse(response.Data);
                    strheader = $('<thead><tr><th class="Collapseheader">Workflow Name</th><th class="Collapseheader">Previous Stage Status</th><th class="Collapseheader">Action</th><th class="Collapseheader">Current Status</th><th class="Collapseheader">Current Updated On</th><th class="Collapseheader">Task Stage</th><th class="Collapseheader">Task</th><th class="Collapseheader">Task Detail</th><th class="Collapseheader">Task Due On <div style=float:right;cursor:pointer><span class="glyphicon glyphicon-remove"  id="closelist" dataval="' + strtaskid + '"  onclick="Hidediv(' + strtaskid +')"/></span></div></th></tr></thead>');
                    strtable.append(strheader);
                    stritem = "<tbody>";
                    $.each(obj, function (i, val) {
                        q1 = q1 + 1;
                        ...
                        if (val.PreviousStageStatus == "Pending" && val.stage != "1") {
                            ...
                            if (val.status == "3") {
                                ...
                                stritem += '<tr><td ' + q1 + '">' + val.WorkFlowName + '</td><td ' + q1 + '">' + val.PreviousStageStatus + '</td><td ' + q1 + '"><a href="#" id="opentrigger" valf="' + val.frmid + '" vals="' + val.stage + '" valt="' + val.TaskId + '" vali="' + val.id + '" valfo="' + val.FormId + '" valw="' + val.wfid + '"  >' + val.stagetext + '</a></td><td>' + val.StatusText + '</td><td>' + strupdatedon + '</td><td ' + q1 + '">' + val.stage + '</td><td ' + q1 + '">' + val.TaskItem + '</td><td ' + q1 + '">' + val.TaskDescription + '</td><td ' + q1 + '">' + formatDatetoIST(val.DueDate) + '</td></tr>';
                            }
                        }
                    });
                    stritem += "</tbody>";
