$(document).ready(function () {
    ...
    function loadtabledata() {
        ...
        $.ajax({
            async: true,
            url: '/api/CallApi/LoadReceiveMessageByCaseId',
            type: 'POST',
            contentType: "application/json; charset=utf-8",
            headers: {
                'token': clientid
                ...
                try {
                    $.each(obj, function (i, val) {
                        ...
                        var $row1 = '<div data-type="accordion-section" id="maindiv' + val.mid + '" class="search" style="cursor: pointer;border-left: 1px solid gray;margin:0 0 5px 0;" >' +
                            ...
                            ' ' +
                            '<div id="cmaindiv1' + val.mid + '" data-type="accordion-section-body" style="cursor:default" >' +
                            ...
                            '<div  id="replyAllbox' + val.mid + '" style="display:none"><textarea id="creplytext' + val.mid + '" style="margin: 0px; width: 476px; height: 74px;" cols="10" rows="5"></textarea>' +
                            ...
                            ' </div>' +
                            ' </div>' +
                            '<section class="content" id="cprint' + val.mid + '"  style="margin-top:10px;">' +
                            '<div id="creplydata' + val.mid + '" class="replydata">' +
                            '</div>' +
                            '<div class="col-md-12">' +
                            '<div class=" ">' +
                            ...
                            '<p> <b>To : </b> <span id="ctouser' + val.mid + '"></span></p>' +
                            ...
                            ' <ul class="mailbox-attachments clearfix" id="cviewattachfile' + val.mid + '" style="' + fileview + '">' +
                            ' </ul>' +
                            ' </div>' +
                            '</section>' +
                            ...
                            '</div > ' +
                            '</div>';
                        $('#accordioncase').append($row1);
                        if (val.mattername == null || val.mattername == "") {
                            $("#ccase" + val.mid).css("display", "none");
                        }
                        else {
                            $("#ccase" + val.mid).css("display", "unset");
                        }
                        ...
                        try {
                            if (String(mfile) != "#") {
                                ...
                                for (var i = 0; i < str_array.length; i++) {
                                    ...
                                    $("#cviewattachfile" + val.mid).append($row);
                                }
                            }
                        }
                        catch (err) {
                    ...
                    try {
                        $.each(obj, function (i, val) {
                            ...
                            var $row1 =
                                ...
                                '<p> <b>To : </b> <span id="ctouserreply' + desired2 + '"></span></p>' +
                                '</div>' +
                                '<div class="mailbox-read-message"> ' + val.mbody + '' +
                                '</div>' +
                                '</div>' +
                                ' <div class="box-footer">' +
                                ' <ul class="mailbox-attachments clearfix" id="cviewattachfilereply' + val.mid + val.id + '" style="' + fileview1 + '">' +
                                '</li>' +
                                ' </ul>' +
                                ' </div>' +
                                ' <br>' +
                            ...
                            try {
                                if (String(mfile) != "") {
                                    ...
                                    for (var i = 0; i < str_array1.length; i++) {
                                        ...
                                        $("#cviewattachfilereply" + val.mid + val.id).append($row1);
                                    }
                                }
                            }
                            catch (err) {
                    ...
                    try {
                        $.each(obj, function (i, val) {
                            ...
                            var $row1 =
                                ...
                                '</div>' +
                                ' <div class="box-footer">' +
                                ' <ul class="mailbox-attachments clearfix" id="cviewattachfilereplyAll' + val.mid + val.id + '" style="' + fileview1 + '">' +
                                '</li>' +
                                ' </ul>' +
                                ' </div>' +
                                ' <br>' +
                            ...
                            try {
                                if (String(mfile) != "") {
                                    ...
                                    for (var i = 0; i < str_array2.length; i++) {
                                        ...
                                        $("#cviewattachfilereplyAll" + val.mid + val.id).append($row2);
                                    }
                                }
                            }
                            catch (err) {
            ...
            success: function (response) {
                if (response.Status == true) {
                    ...
                    $.each(obj, function (i, val) {
                        ...
                        $("#ctouser" + val.MessageId).append(" " + val.shareby + ", ");
                    });
                }
                else {
                    // alert("not found");
                ...
                if (response.Status == true) {
                    ...
                    $.each(obj, function (i, val) {
                        ...
                        if (subdata1 == desireds) {
                            if ($("#ctouserreply" + desireds).text().indexOf(val.shareby) !== -1) {
                            }
                            else {
                                $("#ctouserreply" + desireds).append(" " + val.shareby + ", ");
                            }
                        }
                    });
                    var x = $("#ctouserreply" + desireds).text();
                    x = Array.from(new Set(x.split(','))).toString();
                    $("#ctouserreply" + desireds).text("");
                    $("#ctouserreply" + desireds).append(x);
                }
                else {
                    // alert("not found");
                }
