...
/*Check email validate or not*/
    ...
    $(document).on("click", "#transferpage", function () {
        ...
        var urls = "/" + fcode + "/Employee/LeadSingleView";
        url_redirect({
            url: urls,
            method: "post",
...
/*Load lead call details by lead Id*/
            ...
            $.ajax(
                {
                    ...
                    success: function (data) {
                        ...
                        new PNotify({
                            title: 'Success!',
                            text: ' Lead Call Save Successfully',
                            type: 'success',
                            delay: 3000
                        });
                        loadtabledata();
            ...
            success: function (response) {
                if (response.Status == true) {
                    ...
                    $.each(obj, function (i, val) {
                        q1 = q1 + 1;
                        sorts = sorts + 1;
                        $header.append('<th bgcolor="DIMGRAY" style="text-align: center; padding-top: 12px;" class="class' + q1 + '" onclick="sortTable(' + sorts + ')">' + val.FieldName + ' <span class="fa fa-sort pull-right"></span></th></tr>');
        ...
        var ld12 =     $.ajax({
            ...
            success: function (response) {
                ...
                $.each(obj, function (i, val) {
                    ...
                    $row.append($('<td class="ldn" />').html("<a name=" + val.ldname + "  id='transferpage' href='javascript:void()' sno=" + qty1 + "><span>" + (val.ldname != "" ? val.ldname : '<span style="visibility: hidden;">&nbsp;</span>')));
                    ...
                    $row.append($('<td class="55" />').html("<span><button type='button' val-data='" + val.lid + "' id='addleadcall' class='btn btn-sm btn-success'><span class='glyphicon glyphicon-earphone'></span>&nbsp; </button> " + calldate + ""));
                    $row.append($('<td class="docs" align="center" />').html("<span>" + "<center><a class='btn btn-primary btn-sm ' style='cursor:pointer;" + ldshcss + "' id='filelink'  data-val=" + val.lddocs + "  id-val=" + val.lid + "><span class='glyphicon glyphicon-folder-open'></span>&nbsp;</center></a>"));
