$(document).ready(function () {
    ...
    $(document).on("click", "#transferpage", function () {
        var ftoken = $(this).attr("ftoken");
        var wftoken = $(this).attr("wftoken");
        var cid = $(this).attr("cid");
        var urls = "/" + fcode + "/Employee/CustomFormDataUpdate";
        url_redirect({
            url: urls,
            method: "post",
            data: { "ftoken": ftoken, "wftoken": wftoken, "cid": cid }
        });
    });
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.Status == true) {
                    ...
                    $head1 = $('<th bgcolor="DIMGRAY">Action</th><th bgcolor="DIMGRAY">UserName</th><th bgcolor="DIMGRAY">Date</th>');
                    $header.append($head1);
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $.each(obj, function (i, val) {
                        it = it + 1;
                        var $row = $('<tr />');
                        $row.append($('<td class="s" />').html("<span><a id='transferpage' href='javascript:void()' cid=" + enctypesingle(val.cid) + " ftoken=" + id + " wftoken=" + wftoken + ">Edit</a>"));
