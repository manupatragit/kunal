   ...
   openload();
    ...
    function LoadDirectoryFiles() {
        ...
        var j1 = $.ajax({  async: true, 
            type: "POST",
            url: "/api/callApi/ShareUserDirectoryList",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response1) {
                ...
                $.each(obj, function (i, a) {
                    var tmpdir = a.fname;
                    var replacedata = String("WorkSpace/" + firmid + "/" + a.Firmuser + "/");
                    var dirname = tmpdir.replace(replacedata, '');
                    var dat = a.date_time;
                    var dates1 = dat.substring(0, 10);
                    togsum = togsum + 1;
                    html += '<tr><td class="7"><span class="glyphicon glyphicon-folder-open" aria-hidden="true" style="color:orange"> </span>&nbsp;&nbsp;<a href="/' + fcode + '/Employee/ShareUserFileList/' + a.DirId + ' " style="color: black"><span>' + dirname + '<span></a></td><td width="100px; ">' + dates1 + '</td></td><td>'+a.shareby+'</td></tr>';
                });
