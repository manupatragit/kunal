$(document).ready(function () {
    ...
    /*Export oExcel Contact*/
    $("#excel").click(function () {
        var searchdata = $('#searchdata').val();
        window.location = encodeURI("/employee/ExportoExcelContact?status=true&searchdata=" + searchdata + "&exportfilter=" + exportfilter);
    })
    /*Export Contact in Pdf*/
    $("#pdf").click(function () {
        var searchdata = $('#searchdata').val();
        window.location = encodeURI("/employee/ExportoPdfContact?status=true&searchdata=" + searchdata + "&exportfilter=" + exportfilter);
    })
    /*transfer page*/
    $(document).on("click", "#transferpage", function () {
        var serial = $(this).attr("sno");
        var transferid = $("#hid" + serial).val();
        var urls = "/" + fcode + "/Employee/ContactSingleView";
        url_redirect({
            url: urls,
            method: "post",
    ...
    $("#linkcontact").click(function () {
        window.location = encodeURI("/" + fcode + "/Employee/Configcontact/contact");
    });
        ...
        function deletecontact() {
            ...
            if (result) {
                ...
                if (JSON.stringify(selectedID) != "[]") {
                    ...
                    $.ajax({
                        async: true,
                        url: '/api/EmployeeApi/RemoveContact',
                        ...
                        success: function (response) {
                            ...
                            if (response.Status == true) {
                                ...
                                new PNotify({
                                    title: 'Warning!',
                                    text: ' You are not Authotized to delete this Contact!',
                                    type: 'error',
                                    delay: 2000
                                });
                                closeload();
                    ...
                    new PNotify({
                        title: 'Warning',
                        text: ' You do not have selected any row to delete?',
                        type: 'error',
                        delay: 3000
                    });
                    closeload();
    ...
    var chksflag = true;
    /*After fill for search and press enter key */
    $(document).on('keypress', function (e) {
        if (e.which == 13) {
            if ($("#searchdata").val() != "") {
                document.getElementById("searchdatas").click();
            }
        }
    });
        ...
        var rt1 = $.ajax({
            ...
            success: function (response) {
                if (response.Status == true) {
                    ...
                    $header = $('<thead  >').html('');
                    $head1 = $('<th bgcolor="DIMGRAY"><input type="checkbox" id="select_all"/></th><th bgcolor="DIMGRAY" onclick="sortTable(1)" class="fname">First Name <span class="fa fa-sort pull-right"></span></span></th><th bgcolor="DIMGRAY" onclick="sortTable(2)" class="mname"><span style="padding-right: 20px !important;">Middle Name</span> <span class="fa fa-sort pull-right"></span></th><th bgcolor="DIMGRAY" onclick="sortTable(3)" class="lname"><span style="padding-right: 20px !important;">Last Name </span><span class="fa fa-sort pull-right"></span></th><th bgcolor="DIMGRAY" onclick="sortTable(4)" class="mobno">Mobile <span class="fa fa-sort pull-right"></span></th></th bgcolor="DIMGRAY"><th bgcolor="DIMGRAY"   onclick="sortTable(5)" class="ContactType"><span style="padding-right: 10px !important;">Contact Type</span><span class="fa fa-sort pull-right"></span></th bgcolor="DIMGRAY"><th bgcolor="DIMGRAY" onclick="sortTable(6)" class="cemail"> <span style="padding-right: 20px !important;">Email</span><span class="fa fa-sort pull-right"></span></th><th bgcolor="DIMGRAY" onclick="sortTable(7)" class="assignby"> <span style="padding-right: 20px !important;">Assign by</span><span class="fa fa-sort pull-right"></span></th><th bgcolor="DIMGRAY"  onclick="sortTable(8)"class="tags"><span style="padding-right: 20px !important;">Tags</span><span class="fa fa-sort pull-right"></span></th><th bgcolor="DIMGRAY" onclick="sortTable(9)" class="cadd1"><span style="padding-right: 20px !important;">Address</span> <span class="fa fa-sort pull-right"></span></th><th bgcolor="DIMGRAY" onclick="sortTable(10)" class="cweb"><span style="padding-right: 20px !important;">Website </span><span class="fa fa-sort pull-right"></span></th> ');
                    $header.append($head1);
                    $.each(obj, function (i, val) {
                        q1 = q1 + 1;
                        sort = sort + 1;
                        $header.append('<th bgcolor="DIMGRAY" onclick="sortTable(' + sort + ')"  style="text-align: center; padding-top: 12px;" class="class' + q1 + '">' + val.FieldName + ' <span class="fa fa-sort pull-right"></span></th></tr>');
    ...
    function loadcontactlist(pageindex) {
        ...
        var ld12 = $.ajax({
            async: true,
            url: '/api/EmployeeApi/SpContactData',
            data: formdata,
            processData: false,
            contentType: false,
            type: 'POST',
            success: function (response) {
                ...
                $.each(obj, function (i, val) {
                    ...
                    if (usedNames.indexOf(val.assignuserby) == -1) {
                        $("#fassign").append("<option value=" + val.assignuserby + ">" + val.assignuserby + "</option>");
                        usedNames.push(val.assignuserby);
                    }
                    if (val.ContactType != "") {
                    ...
                    // var whtsdata = "Name:" + (val.fname != null ? val.fname : '') + " " + (val.mname != null ? val.mname : '') + " " + (val.lname != null ? val.lname : '') + ", Mobile:" + (val.mobno != null ? val.mobno : '') + ", Email:" + (val.cemail != null ? val.cemail : '') + ", Address:" + (val.cadd1 != null ? val.cadd1 : '') + ", Website:" + (val.cwebsite != null ? val.cwebsite : '');
                    ...
                    $row.append($('<td class="fname" />').html("<a name=" + val.fname + " id='transferpage' href='javascript:void()' sno=" + qty + "><span>" + (val.fname != "" ? val.fname : '<span style="visibility: hidden;">&nbsp;</span>')));
                    ...
                    $row.append($('<td class="auser" />').html("<span>" + (val.assignuserby != null ? val.assignuserby : '<span style="">&nbsp;</span>')));
