$(document).ready(function () {
    ...
    /*show bruid crmb load contact permission*/
    ...
    $(document).on("click", "#removedirfile", function () {
        ...
        if (result) {
            $.ajax({
                async: true,
                type: "POST",
                url: "/api/CallApi/RemoveUserDirectoryFile",
                headers: {
                    'ffid': ffid,
                    'ffdir': ffdir
                },
...
/*Create files*/
    ...
    function LoadDirectoryFiles(pageindex) {
        ...
        var j1 = $.ajax({
            async: true,
            type: "POST",
            url: "/api/callApi/AssignUserFileListbyrowid",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            data: formData,
            contentType: false,
            ...
            success: function (response1) {
                ...
                $.each(obj, function (i, a) {
                    ...
                    try {
                        ...
                        html += '<tr><td class="7"><i class="' + ficon + '" style="font-size:14px;color:' + icolor + '"></i><span style="' + pointer + '" id="' + idtemp + '"  value="' + last + '"  href="' + openpath + '" urltype="' + urlstype + '"> ' + a.fname + '</span></a></td><td width="100px; ">' + dates1 + '</td><td>' + last + '</td><td><a class="glyphicon glyphicon-download-alt" style="color:green; cursor:pointer" download="' + a.fname + '" id="downloadfile" href="' + downloadpath + '" values="' + a.id + '"> </a>&nbsp;&nbsp;&nbsp;<btn class="" style=" cursor:pointer;margin-top:0px; color:red;' + ncss + '" data-toggle="modal" id="removedirfile" directnamefull="' + downloadpath + '" value="' + a.id + '" pathid="path' + a.id + '"></span> <span class="glyphicon glyphicon-trash" ></span></btn></td><td>' + a.assignby + '</td></tr>';
                    }
                    catch (err) {
                        alert(err.message);
                    }
