$(document).ready(function () {
    ...
    /*Remove archive message base on selected id*/
    ...
    /*Get archive message details by page number*/
    ...
    loadtabledata(pageindex);
    /*load archive message table data*/
    var mids = 0;
    function loadtabledata(pageindex) {
        ...
        $.ajax({
            async: true,
            url: '/api/CallApi/LoadArchiveMessagebyrowid',
            data: formdata,
            processData: false,
            contentType: false,
            type: 'POST',
                ...
                try {
                    $.each(obj, function (i, val) {
                        ...
                        var $row1 = '<div data-type="accordion-section" id="maindiv' + val.mid + '" class="search" style="cursor: pointer;border-left: 1px solid gray;border-bottom: 1px solid lightgray; overflow:hidden;margin:0 0 5px 0;" >' +
                            ...
                            ' <button type="button" class="btn btn-warning" id="print1" data-toggle="tooltip" title="Print" idval="' + val.mid + '"><i class="glyphicon glyphicon-print"></i>&nbsp;</button>' +
                            ' <button type="button" id="unarchivemsg" idval="' + val.mid + '" class="btn btn-primary"><i class="fa fa-archive"></i> Un Archive</button>' +
                            ...
                            '</div>' +
                            '<div style="widht:100%">' +
                            '<input type="button" id="savereplyAll" idval="' + val.mid + '" userval="' + val.userid + '" class="btn save ash' + val.mid + '" value="Send All">&nbsp;&nbsp;&nbsp;' +
                            '<input type="button" id="savereply" idval="' + val.mid + '" userval="' + val.userid + '" class="btn save sh' + val.mid + '" value="Send">&nbsp;&nbsp;&nbsp;' +
                            '<input type="button" id="closereply" class="btn save" value="Close" idval="' + val.mid + '">' +
                            '</div>' +
                            ' </div>' +
                            ' </div>' +
                            ...
                            '<div class="mailbox-read-message" style="margin-top:10px;"> ' + val.mbody + '' +
                            '</div>' +
                            '</div></div></div>' +
    ...
    /*Get unarchive message*/
    $(document).on("click", "#unarchivemsg", function () {
        if (confirm('Are you sure to unarchive this message?')) {
            var id = $(this).attr("idval");
            var formData = new FormData();
            ...
            $.ajax(
                {
                    type: "POST",
                    url: "/api/callApi/UnArchiveMessage", // Controller/View
                    data: formData,
                    contentType: false,
                    processData: false,
                    success: function (data) {
                        ...
                        new PNotify({
                            title: 'Success!',
                            text: ' Message unarchived successfully',
                            type: 'success',
                            delay: 3000
                        });
    ...
    /*Load archive message reply details*/
