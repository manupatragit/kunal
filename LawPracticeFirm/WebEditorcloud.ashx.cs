...
namespace LawPracticeFirm
{
    ...
    public class WebEditorcloud : IHttpHandler
    {
        ...
        private static void Track(HttpContext context)
        {
            ...
            LogService("1=" + userAddress);


            var fileName = "E2bdADS_" + context.Request["fileName"];
            var dfileName = "D2bdADS_" + context.Request["fileName"];
            var fileNameenc = context.Request["fileName"];
            LogService("fileName2=" + fileName);
            var dname = context.Request["dtoken"];
            LogService("dname3=" + dname);
            var firmids = context.Request["ftoken"];
            LogService("firmids4=" + firmids);
            var userids = context.Request["utoken"];


            LogService("userids5=" + userids);
            var Token = context.Request["Token"];
            LogService("Token=" + Token);
            var storagePath = DocManagerHelper.StoragePathEditFileAzure(fileName, userAddress, dname, firmids, userids);

            var storagePathcloud = DocManagerHelper.StoragePathEditFilecloud(fileName, userAddress, dname, firmids, userids);
            var newstoragePath = storagePath + fileName;
            var azurestoragePathenc = storagePathcloud;
            var storagePathenc1 = storagePath + dfileName;

            LogService("Storage Path=" + storagePath);
            LogService("Storage storagePathcloud=" + storagePathcloud);
            LogService("Storage storagePathenc=" + azurestoragePathenc);
            LogService("Storage newstoragePath=" + newstoragePath);
            ...
            switch (status)
            {
                ...
                case TrackerStatus.Corrupted:
                    ...
                    {
                        // saving a document
                        saved = TrackManager.processSave(fileData, fileName, userAddress, newstoragePath, storagePathenc1, azurestoragePathenc);
                        try
                        {
                            LogService("firmids=" + firmids);
                            LogService("userids=" + Token);
                            AzureDocument.creatroot(firmids, userids);

                        }

                        catch (Exception err)
                        {
                            LogService("during ceate root edit=" + err.Message + "&" + err.StackTrace + "&" + err.InnerException);
                        }

                        try
                        {

                            AzureDocument.CreateDocEditDestination(azurestoragePathenc, firmids, userids);

                        }

                        catch (Exception err)
                        {
                            LogService("during ceate storage destination path=" + err.Message + "&" + err.StackTrace + "&" + err.InnerException);
                        }

                        try
                        {

                            var done = AzureDocument.filecreateafteredit(storagePathenc1, azurestoragePathenc, fileNameenc, firmids, userids);
                            //update docsize after update
                            var db = new LawPracticeEntities();
                            long Docsize = 0;
                            //get docsize
                            FileInfo fi = new FileInfo(storagePathenc1);
                            Docsize = fi.Length;
                            LogService("firmids=" + firmids);
                            LogService("Token=" + Token);
                            LogService("docsize=" + Docsize);
                            var cnt = db.sp_UpdateDocSize(firmids, Token, Docsize);
                            //Added for notifications
                            var noti = db.sp_UpdateEventForOnlineDoc(firmids, Token, userids);
                            LogService("done=" + done);
                        }

                        catch (Exception err)
                        {
                            LogService("during edit encty11=" + err.Message + "&" + err.StackTrace + "&" + err.InnerException);
                        }
                    }
                    catch (Exception)
                    {
                ...
                case TrackerStatus.CorruptedForceSave:
                    try
                    {
                        ...
                        saved = TrackManager.processForceSave(fileData, fileName, userAddress, newstoragePath, storagePathenc1, azurestoragePathenc);
                        try
                        {
